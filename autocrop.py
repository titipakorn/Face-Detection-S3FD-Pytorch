...
def crop_positions(
    ...
    padUp = 5 if (padUp is False or padUp < 0) else padUp
    padDown = 5 if (padDown is False or padDown < 0) else padDown
    padLeft = 5 if (padLeft is False or padLeft < 0) else padLeft
    padRight = 5 if (padRight is False or padRight < 0) else padRight
    ...
    xpad = (width_crop - (w-x)) / 2
    ypad = (height_crop - (h-y)) / 2

    # Calc. positions of crop
    h1 = float(x - (xpad * padLeft / (padLeft + padRight)))
    h2 = float(w + (xpad * padRight / (padLeft + padRight)))
    v1 = float(y - (ypad * padUp / (padUp + padDown)))
    v2 = float(h + (ypad * padDown / (padUp + padDown)))
...
def crop(
    ...
    for i in range(detections.size(1)):
        ...
        while detections[0, i, j, 0] >= thresh:
            ...
            if(detections[0, i, j, 0]>max_thresh):
                max_thresh=detections[0, i, j, 0]
                max_loc=[i,j]
            j+=1
    # Handle no faces
    if j == 0:
    ...
    x, y, w, h = (detections[0, max_loc[0], max_loc[1], 1:] * scale).cpu().numpy()
...
if __name__=="__main__":
    ...
    net.load_state_dict(torch.load('weights/sfd_face.pth',map_location='cpu'))
    net.eval()
    if use_cuda:
        net.cuda()
